...
docker start registry || docker run -d -p 5555:5000 --name registry registry:2
...
docker stop demo2 && docker run -p 12345:5000 -d --rm --pull always --name demo2 localhost:5555/demo2:latest